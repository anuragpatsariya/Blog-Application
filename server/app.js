...
var topic = request.body.btopic;
var category = request.body.bcategory;
var filepath = request.body.bfilpath;
var bcontent = request.body.bcontent;
var filename = request.body.filename;
var file = request.files.file;
file.mv(`${__dirname}/uploaded_images/${request.body.filename}`, function(err) {
  if(err) {
...
server.get("/all_blogs", function (request, response){
  ...
  var bucket = cluster.openBucket('blog_data', function(err){
    ...
    var q = N1qlQuery.fromString('select username, topic, content, filepath, category from blog_data');
    var count = 0;
